...
For restricted networks, `AJIASU_COMPOSE_REGISTRY_MIRROR` may replace only the transport registry while retaining every locked base-image digest. The default BuildKit SBOM generator is locked in `build/compose-images.lock`; `AJIASU_COMPOSE_SBOM_SCANNER` may override it only with another digest-pinned generator image. `AJIASU_COMPOSE_GO_PROXY` selects the Go module proxy, and `AJIASU_COMPOSE_CARGO_REGISTRY_INDEX` accepts an HTTPS Cargo sparse registry ending in `/`; `go.sum` and `Cargo.lock` integrity checks remain active. `AJIASU_COMPOSE_TRIVY_CACHE_DIRECTORY` may point at an existing, pre-warmed cache; Trivy still checks its metadata and refreshes stale databases. These settings do not disable SBOM, provenance, vulnerability, or secret gates; image scans retry transient failures up to three times against one shared cache.
