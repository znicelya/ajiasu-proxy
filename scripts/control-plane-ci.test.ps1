...
foreach ($workflowFixture in @(
    @{ Name = 'control-plane'; Path = '../.github/workflows/control-plane-ci.yml'; Command = 'run: ./scripts/control-plane-ci.ps1' },
    @{ Name = 'runner'; Path = '../.github/workflows/runner-ci.yml'; Command = 'run: ./scripts/ci.ps1' }
)) {
    $workflowPath = Join-Path $PSScriptRoot $workflowFixture.Path
    $workflow = Get-Content -LiteralPath $workflowPath -Raw
    $verifyJobMatch = [regex]::Match($workflow, '(?ms)^  verify:\r?\n(?<job>.*?)(?=^  [a-zA-Z0-9_-]+:\r?$)')
    if (-not $verifyJobMatch.Success) {
        throw "$($workflowFixture.Name) workflow must define the verify job"
    }

    $verifyJob = $verifyJobMatch.Groups['job'].Value
    if (-not $verifyJob.Contains('docker/setup-qemu-action@')) {
        throw "$($workflowFixture.Name) verify job must install QEMU before its multiarch image build"
    }
    if (-not $verifyJob.Contains('docker/setup-buildx-action@')) {
        throw "$($workflowFixture.Name) verify job must install a multi-platform Buildx builder"
    }
    if ($verifyJob.IndexOf('docker/setup-buildx-action@') -gt $verifyJob.IndexOf($workflowFixture.Command)) {
        throw "$($workflowFixture.Name) verify job must configure Buildx before running the CI script"
    }
}
